...
CREATE OR REPLACE FUNCTION "public"."check_assignment_overallocation"(
        "_assignment_id" "uuid",
        "_person_id" "uuid",
        "_start_date" "date",
        "_end_date" "date",
        ...
        "overallocated_date" "date",
        "total_allocation" double precision
    ) LANGUAGE "plpgsql" AS $$
DECLARE d date;
current_total double precision;
existing_sum double precision;
BEGIN -- Iterar por cada día en el rango
FOR d IN
SELECT generate_series(_start_date, _end_date, interval '1 day')::date LOOP -- Calcular la asignación existente para este día
SELECT COALESCE(SUM(a.allocation), 0) INTO existing_sum
FROM assignments a
...
IF current_total > 1.0 THEN overallocated_date := d;
...
END;
$$;
ALTER FUNCTION "public"."check_assignment_overallocation"(
    "_assignment_id" "uuid",
    "_person_id" "uuid",
    "_start_date" "date",
    "_end_date" "date",
...
CREATE OR REPLACE FUNCTION "public"."generate_llnn_code"() RETURNS "text" LANGUAGE "plpgsql" AS $$
DECLARE letters TEXT := 'ABCDEFGHIJKLMNÑOPQRSTUVWXYZ';
result TEXT := '';
digit1 INT;
digit2 INT;
BEGIN -- Random two uppercase letters
result := substr(letters, trunc(random() * 26 + 1)::int, 1) || substr(letters, trunc(random() * 26 + 1)::int, 1);
-- Random two digits
...
CREATE OR REPLACE FUNCTION "public"."generate_unique_llnn_code"() RETURNS "text" LANGUAGE "plpgsql" AS $$
DECLARE new_code TEXT;
tries INT := 0;
BEGIN LOOP new_code := generate_llnn_code();
...
OR tries > 100;
END LOOP;
IF tries > 100 THEN RAISE EXCEPTION 'Failed to generate unique code after 100 attempts';
END IF;
RETURN new_code;
END;
$$;
...
CREATE OR REPLACE FUNCTION "public"."get_audit_logs_for_project_code"("p_project_code" "text") RETURNS SETOF "public"."audit_log" LANGUAGE "sql" SECURITY DEFINER AS $$ with target_project as (
        select id
        from projects
...
CREATE OR REPLACE FUNCTION "public"."get_audit_logs_for_project_code"(
        "p_project_code" "text",
        "p_limit" integer DEFAULT 20,
        "p_offset" integer DEFAULT 0
    ) RETURNS SETOF "public"."audit_log" LANGUAGE "sql" SECURITY DEFINER AS $$ with target_project as (
        select id
        from projects
        where project_code = p_project_code
    )
...
ALTER FUNCTION "public"."get_audit_logs_for_project_code"(
    "p_project_code" "text",
    "p_limit" integer,
    "p_offset" integer
) OWNER TO "postgres";
CREATE OR REPLACE FUNCTION "public"."get_project_activity_logs"("p_project_id" "uuid") RETURNS TABLE(
        "id" "uuid",
        "user_id" "uuid",
        "display_name" "text",
        "email" "text",
...
CREATE OR REPLACE FUNCTION "public"."log_activity_change"() RETURNS "trigger" LANGUAGE "plpgsql" AS $$
DECLARE action_type TEXT;
affected_id UUID;
metadata JSONB;
BEGIN -- Determine action
...
CREATE OR REPLACE FUNCTION "public"."log_assignment_change"() RETURNS "trigger" LANGUAGE "plpgsql" AS $$DECLARE user_id UUID := current_setting('app.current_user_id', true)::UUID;
user_name TEXT;
user_email TEXT;
...
CREATE OR REPLACE FUNCTION "public"."log_generic_changes"() RETURNS "trigger" LANGUAGE "plpgsql" AS $$
...
if TG_OP = 'INSERT' then new_vals := hstore(new);
...
elsif TG_OP = 'DELETE' then old_vals := hstore(old);
...
elsif TG_OP = 'UPDATE' then old_vals := hstore(old);
new_vals := hstore(new);
keys := akeys(new_vals - old_vals);
foreach key in array keys loop if old_vals->key is distinct
from new_vals->key then
insert into audit_log (
        table_name,
        record_id,
        field_name,
...
CREATE OR REPLACE FUNCTION "public"."log_person_change"() RETURNS "trigger" LANGUAGE "plpgsql" AS $$DECLARE user_id UUID := current_setting('app.current_user_id', true)::UUID;
user_name TEXT;
user_email TEXT;
BEGIN -- Fetch name and email from auth.users.raw_user_meta_data
SELECT raw_user_meta_data->>'name',
...
CREATE OR REPLACE FUNCTION "public"."log_project_change"() RETURNS "trigger" LANGUAGE "plpgsql" AS $$DECLARE user_id UUID := current_setting('app.current_user_id', true)::UUID;
user_name TEXT;
user_email TEXT;
BEGIN -- Fetch name and email from auth.users.raw_user_meta_data
SELECT raw_user_meta_data->>'name',
...
CREATE OR REPLACE FUNCTION "public"."ocupation_report_between"("initial_date" "date", "final_date" "date") RETURNS TABLE(
        "assignment_id" "uuid",
        "person_id" "uuid",
        "project_id" "uuid",
...
where a.start_date <= ocupation_report_between.final_date
...
CREATE OR REPLACE FUNCTION "public"."update_updated_at_date"() RETURNS "trigger" LANGUAGE "plpgsql" AS $$ BEGIN NEW.updated_at := CURRENT_DATE;
RETURN NEW;
END;
$$;
...
CREATE TABLE IF NOT EXISTS "public"."assignments" (
    ...
    CONSTRAINT "assignments_allocation_check" CHECK (
        (
            ("allocation" >= (0.0)::double precision)
            AND ("allocation" <= (100.0)::double precision)
        )
    )
);
...
CREATE TABLE IF NOT EXISTS "public"."assignments_history" (
    ...
    "changed_by_name" "text",
    "changed_by_email" "text",
    CONSTRAINT "assignments_history_action_check" CHECK (
        (
            "action" = ANY (
...
CREATE TABLE IF NOT EXISTS "public"."people" (
    ...
    CONSTRAINT "people_status_check" CHECK (
        (
            ("status")::"text" = ANY (
                ARRAY [('Active'::character varying)::"text", ('Paused'::character varying)::"text", ('Terminated'::character varying)::"text"]
            )
        )
    ),
    CONSTRAINT "people_type_check" CHECK (
        (
            ("type")::"text" = ANY (
                ARRAY [('Internal'::character varying)::"text", ('External'::character varying)::"text"]
            )
        )
    )
);
...
CREATE TABLE IF NOT EXISTS "public"."people_history" (
    ...
    "changed_by_name" "text",
    "changed_by_email" "text",
    CONSTRAINT "people_history_action_check" CHECK (
        (
            "action" = ANY (
                ARRAY ['INSERT'::"text", 'UPDATE'::"text", 'DELETE'::"text"]
            )
...
CREATE TABLE IF NOT EXISTS "public"."projects" (
    ...
    CONSTRAINT "projects_project_code_check" CHECK (
        ("project_code" ~ '^[A-Za-z]{2}[0-9]{2}$'::"text")
    ),
    CONSTRAINT "projects_status_check" CHECK (
        (
            ("status")::"text" = ANY (
                (
                    ARRAY ['In Progress'::character varying, 'Finished'::character varying, 'On Hold'::character varying, 'Not Started'::character varying]
                )::"text" []
            )
        )
    )
...
CREATE TABLE IF NOT EXISTS "public"."projects_history" (
    ...
    "changed_by_name" "text",
    "changed_by_email" "text",
    CONSTRAINT "projects_history_action_check" CHECK (
        (
            "action" = ANY (
                ARRAY ['INSERT'::"text", 'UPDATE'::"text", 'DELETE'::"text"]
            )
    ...
    CONSTRAINT "projects_history_project_code_check" CHECK (
        ("project_code" ~ '^[A-Za-z]{2}[0-9]{2}$'::"text")
    )
);
...
CREATE OR REPLACE TRIGGER "set_updated_at_date" BEFORE
UPDATE ON "public"."assignments" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_date"();
CREATE OR REPLACE TRIGGER "trg_audit_generic"
AFTER
INSERT
    OR DELETE
    OR
UPDATE ON "public"."assignments" FOR EACH ROW EXECUTE FUNCTION "public"."log_generic_changes"();
CREATE OR REPLACE TRIGGER "trg_audit_generic"
AFTER
INSERT
    OR DELETE
    OR
UPDATE ON "public"."people" FOR EACH ROW EXECUTE FUNCTION "public"."log_generic_changes"();
CREATE OR REPLACE TRIGGER "trg_audit_generic"
AFTER
INSERT
    OR DELETE
    OR
UPDATE ON "public"."projects" FOR EACH ROW EXECUTE FUNCTION "public"."log_generic_changes"();
CREATE OR REPLACE TRIGGER "trigger_log_assignments_change"
AFTER
INSERT
    OR DELETE
    OR
UPDATE ON "public"."assignments" FOR EACH ROW EXECUTE FUNCTION "public"."log_assignment_change"();
CREATE OR REPLACE TRIGGER "trigger_log_person_change"
AFTER
INSERT
    OR DELETE
    OR
UPDATE ON "public"."people" FOR EACH ROW EXECUTE FUNCTION "public"."log_person_change"();
CREATE OR REPLACE TRIGGER "trigger_log_projects_change"
AFTER
INSERT
    OR DELETE
    OR
UPDATE ON "public"."projects" FOR EACH ROW EXECUTE FUNCTION "public"."log_project_change"();
ALTER TABLE ONLY "public"."activity_logs"
ADD CONSTRAINT "activity_logs_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE
SET NULL;
ALTER TABLE ONLY "public"."assignments"
...
ALTER TABLE ONLY "public"."projects"
ADD CONSTRAINT "projects_client_id_fkey" FOREIGN KEY ("client_id") REFERENCES "public"."clients"("id") ON DELETE
SET NULL;
CREATE POLICY "Allow access to authenticated users" ON "public"."assignments" TO "authenticated" USING (
    (
        "set_config"(
...
CREATE POLICY "Allow access to authenticated users" ON "public"."client_contacts" TO "authenticated" USING (
    (
        "set_config"(
            'app.current_user_id'::"text",
            ("auth"."uid"())::"text",
...
CREATE POLICY "Allow access to authenticated users" ON "public"."clients" TO "authenticated" USING (
    (
        "set_config"(
            'app.current_user_id'::"text",
            ("auth"."uid"())::"text",
...
CREATE POLICY "Allow access to authenticated users" ON "public"."people" TO "authenticated" USING (
    (
        "set_config"(
            'app.current_user_id'::"text",
            ("auth"."uid"())::"text",
...
CREATE POLICY "Allow access to authenticated users" ON "public"."projects" TO "authenticated" USING (
    (
        "set_config"(
            'app.current_user_id'::"text",
            ("auth"."uid"())::"text",
...
CREATE POLICY "Authenticated can read logs" ON "public"."audit_log" FOR
SELECT TO "authenticated" USING (true);
CREATE POLICY "Policy with security definer functions" ON "public"."assignments_history" TO "authenticated" USING (
    (
        "set_config"(
            'app.current_user_id'::"text",
            ("auth"."uid"())::"text",
...
CREATE POLICY "Policy with security definer functions" ON "public"."people_history" TO "authenticated" USING (
    (
        "set_config"(
            'app.current_user_id'::"text",
            ("auth"."uid"())::"text",
...
CREATE POLICY "Policy with security definer functions" ON "public"."projects_history" TO "authenticated" USING (
    (
        "set_config"(
            'app.current_user_id'::"text",
            ("auth"."uid"())::"text",
...
CREATE POLICY "Trigger-only insert" ON "public"."audit_log" FOR
INSERT TO "authenticated" WITH CHECK (
        (
            "current_setting"('app.current_user_id'::"text", true) IS NOT NULL
        )
    );
CREATE POLICY "Authenticated can insert projects" ON "public"."projects" FOR
...
GRANT ALL ON FUNCTION "public"."check_assignment_overallocation"(
        "_assignment_id" "uuid",
        "_person_id" "uuid",
        "_start_date" "date",
        "_end_date" "date",
...
GRANT ALL ON FUNCTION "public"."get_audit_logs_for_project_code"(
        "p_project_code" "text",
        "p_limit" integer,
        "p_offset" integer
    ) TO "anon";
